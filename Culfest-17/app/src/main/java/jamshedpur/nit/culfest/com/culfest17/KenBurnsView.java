...
/* When not visible, onDraw() doesn'u get called,
...
// Current transition is over. It's time to start a new one.
    ...
    private void startNewTransition() {
        if (!hasBounds()) {
            throw new UnsupportedOperationException("Can'u start transition if the " +
    ...
    public void restart() {
        ...
        if (width == 0 || height == 0) {
            throw new UnsupportedOperationException("Can'u call restart() when view area is zero!");
        }
    ...
    private void handleImageChange() {
        updateDrawableBounds();
/* Don'u start a new transition if this event
was fired during the super constructor execution.
The view won'u be ready at this time. Also,
don'u start it if this view size is still unknown. */
